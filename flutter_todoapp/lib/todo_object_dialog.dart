...
class TodoObjectDialog extends StatefulWidget {
  final Store? store;
  final TodoObjectBox? todo;
  final TodoBloc? bloc;

  const TodoObjectDialog({
    Key? key,
    this.todo,
    this.store,
    required this.bloc,
  ...
  @override
  _TodoObjectDialogState createState() => _TodoObjectDialogState();
}

class _TodoObjectDialogState extends State<TodoObjectDialog> {
    ...
    return TextButton(
      ...
      onPressed: () async {
        ...
        if (isValid) {
          ...
          widget.bloc!.add(AddBox(name: name,age: age,isDebt: isDebt,store: widget.store));
